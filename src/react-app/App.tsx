...
const Icon = ({ d, size = 18 }: IconProps) => (
...
export default function PDFReader() {
  ...
  useEffect(() => {
    ...
    const el = paraListRef.current.querySelector<HTMLElement>(`[data-para="${activePara}"]`);
  ...
  const playFrom = (fromIdx: number) => {
    ...
    utter.onend   = () => { setTtsState("idle"); setActivePara(-1); };
    utter.onerror = () => { setTtsState("idle"); setActivePara(-1); };
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    pageInfo:    { fontSize: 13, color: "#888", minWidth: 100, textAlign: "center", letterSpacing: "0.05em" },
  ...
  const ttsBtnStyle = (active: boolean, color = "#c9a84c"): React.CSSProperties => ({
    flex: 1, padding: "10px 0", border: "none", borderRadius: 6, cursor: "pointer",
    fontFamily: "'Georgia',serif", fontSize: 12, display: "flex", alignItems: "center",
    justifyContent: "center", gap: 6, transition: "all 0.15s",
    background: active ? color : "#1a1a1f", color: active ? "#0a0a0c" : "#888",
  });

  const navBtnStyle = (disabled: boolean): React.CSSProperties => ({
    background: "none", border: "1px solid #2a2a30", color: disabled ? "#333" : "#aaa",
    borderRadius: 6, padding: "6px 10px", cursor: disabled ? "not-allowed" : "pointer",
    display: "flex", alignItems: "center", transition: "all 0.15s",
  });

  const paraItemStyle = (isActive: boolean): React.CSSProperties => ({
    padding: "8px 10px", borderRadius: 6, fontSize: 11, lineHeight: "1.65",
  ...
  const statusDotStyle = (st: TtsState): React.CSSProperties => ({
    width: 8, height: 8, borderRadius: "50%",
    background: st === "playing" ? "#4caf50" : st === "paused" ? "#c9a84c" : "#333",
    boxShadow: st === "playing" ? "0 0 6px #4caf50" : "none",
    transition: "all 0.3s",
  });
    ...
    <div style={s.root}>
      ...
      <header style={s.header}>
        ...
        <div style={s.statusRow}>
          <div style={statusDotStyle(ttsState)} />
      ...
      <div style={s.body}>
          ...
          <div>
            ...
            >
              ...
              <div style={s.uploadText}>
                {pdfReady ? "Drop a PDF here\nor click to browse" : "Loading PDF engine…"}
              </div>
            </div>
          ...
          <div>
            <div style={s.secTitle}>
              <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <SpeakerIcon /> Text-to-Speech
              </span>
            </div>
            <div style={s.ttsButtons}>
              <button
                style={ttsBtnStyle(ttsState === "playing", "#4caf50")}
                onClick={ttsState === "idle" ? () => playFrom(0) : pauseTts}
                disabled={!pdfDoc}
              >
              ...
              <button style={ttsBtnStyle(false)} onClick={stopTts} disabled={ttsState === "idle"}>
                <StopIcon />
              </button>
            </div>

            {voices.length > 0 && (
              <select style={s.select} value={selectedVoice}
            ...
            <div style={s.sliderGroup}>
              <div style={s.sliderLabel}>
                <span>Speed</span>
                <span style={{ color: "#c9a84c" }}>{ttsRate.toFixed(1)}×</span>
              </div>
            ...
            <div style={s.sliderGroup}>
              <div style={s.sliderLabel}>
                <span>Pitch</span>
                <span style={{ color: "#c9a84c" }}>{ttsPitch.toFixed(1)}</span>
              </div>
              ...
              <div ref={paraListRef} style={s.paraList}>
                {paragraphs.map((p, i) => (
                  <div
                    key={i}
                    data-para={i}
                    style={paraItemStyle(activePara === i)}
                    onClick={() => playFrom(i)}
                    title="Click to read from here"
                  >
                    {activePara === i && (
                      <span style={{ fontSize: 9, color: "#c9a84c", letterSpacing: "0.1em", display: "block", marginBottom: 3 }}>
                        ▶ NOW READING
                      </span>
                    )}
                    {p}
        ...
        <main style={s.viewer}>
          {pdfDoc && (
            <div style={s.toolbar}>
              <button style={navBtnStyle(pageNum <= 1)} onClick={prevPage} disabled={pageNum <= 1}>
                <ChevronLeft />
              </button>
              <div style={s.pageInfo}>
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (v >= 1 && v <= totalPages) { stopTts(); setPageNum(v); }
                  }}
                />
                <span style={{ marginLeft: 6 }}>/ {totalPages}</span>
              </div>
              <button style={navBtnStyle(pageNum >= totalPages)} onClick={nextPage} disabled={pageNum >= totalPages}>
                <ChevronRight />
              </button>
          ...
          <div style={s.canvasWrap}>
            {pdfDoc ? (
              ...
              <div style={s.emptyState}>
                ...
                <div style={{ fontFamily: "'Georgia',serif", fontSize: 22, color: "#333", letterSpacing: "0.04em" }}>
                  No document open
                </div>
                <div style={{ fontSize: 13, color: "#444", letterSpacing: "0.06em" }}>
